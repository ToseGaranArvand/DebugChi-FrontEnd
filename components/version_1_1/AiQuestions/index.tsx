...
export default function AiQuestion({ question }: Props) {
  ...
  const handleSelectCategory = (item: Main) => {
    ...
    if (
      item.category_name.includes("رفع اشکال") ||
      item.category_name.includes("دیباگ")
    ) {
    ...
    console.log(
      `Selected category: ${item.category_name}, requestType: ${newRequestType}`
    );
  };
  ...
  const handleConfirmRequest = async () => {
    ...
    try {
      ...
      const confirmationMessage: Message = {
        ...
        displayText:
          "درخواست شما با موفقیت تأیید شد! در انتظار پذیرش یک متخصص...",
        isAI: true,
      };
      ...
      >
        <div
          ...
          style={{
            ...
            maxWidth: "600px",
            background:
              "linear-gradient(180deg, rgba(186, 186, 186, 0.32) 19.72%, rgba(48, 48, 48, 0) 100%)",
            borderRadius: "16px",
            padding: "1px",
